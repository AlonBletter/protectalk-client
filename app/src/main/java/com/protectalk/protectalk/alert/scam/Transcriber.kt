...
class Transcriber {
    ...
    /**
     * Transcribes a prepared WAV file using Google Speech-to-Text API
     * Compatible with API level 24+
     *
     * @param wavFile The WAV file to transcribe (should be from app cache)
     */
    suspend fun transcribeWavFile(wavFile: File): String = withContext(Dispatchers.IO) {
        Log.d(TAG, "🎤 Starting transcription of: ${wavFile.absolutePath}")

        if (!wavFile.exists()) {
            Log.e(TAG, "❌ WAV file does not exist: ${wavFile.absolutePath}")
            return@withContext "WAV file not found."
        }

        val audioBytes: ByteArray = wavFile.readBytes()
        val audioBase64: String = Base64.encodeToString(audioBytes, Base64.NO_WRAP)

        // Since we're working with WAV files from cache, use LINEAR16 encoding
        val encodingType = "LINEAR16"
    ...
    /**
     * Legacy method - finds latest recording and transcribes it
     * @deprecated Use transcribeWavFile(File) with RecordingFinder.findAndPrepareLatestRecording() instead
     */
    @Deprecated("Use transcribeWavFile(File) with RecordingFinder.findAndPrepareLatestRecording() instead")
    suspend fun transcribeFromLatestFile(): String = withContext(Dispatchers.IO) {
        ...
        val operationId: String = JSONObject(body).getString("name")
        val operationStatusUrl =
            "https://speech.googleapis.com/v1/operations/$operationId?key=${getGoogleApiKey()}"
