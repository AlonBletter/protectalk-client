package com.protectalk.protectalk.data.remote.transcription
...
class Transcriber {
    ...
    suspend fun transcribeFromLatestFile(): String = withContext(Dispatchers.IO) {
        ...
        val inputAudioFile: File = when (latestRecordingFile.extension.lowercase()) {
            "m4a" -> {
                val wavFile = File(
                    latestRecordingFile.parent,
                    "${latestRecordingFile.nameWithoutExtension}.wav"
                )
                val conversionSuccessful: Boolean =
                    AudioConverter.convertM4aToWav(latestRecordingFile, wavFile)
        ...
        repeat(MAX_POLLING_ATTEMPTS) {
            ...
            httpClient.newCall(Request.Builder().url(operationStatusUrl).build()).execute()
                .use { pollingResponse ->
                    ...
                    if (pollingResponse.isSuccessful) {
                        ...
                        if (pollingJson.optBoolean("done", false)) {
                            ...
                            val resultsJsonArray =
                                responseJson.optJSONArray("results") ?: return@use

                            val diarizedWordsList = (0 until resultsJsonArray.length())
                                .asSequence()
                                .flatMap { i ->
                                    val alternativesArray = resultsJsonArray.getJSONObject(i)
                                        .getJSONArray("alternatives")
                                    (0 until alternativesArray.length()).asSequence().flatMap { j ->
                                        val wordsArray =
                                            alternativesArray.getJSONObject(j).optJSONArray("words")
                                        wordsArray?.let { wordArr ->
                                            (0 until wordArr.length()).asSequence()
                                                .map { k -> wordArr.getJSONObject(k) }
                                        } ?: emptySequence()
                                    }
                                }
                        ...
                        Log.e(
                            TAG,
                            "Polling failed: ${pollingResponse.code} / ${pollingResponse.body?.string()}"
                        )
                    }
                }
        }
